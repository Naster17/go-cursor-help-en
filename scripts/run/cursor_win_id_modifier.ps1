...
if ($executeMode -eq "MODIFY_ONLY") {
    ...
    Write-Host "$BLUE  1️⃣  Detect Cursor config file$NC"
    Write-Host "$BLUE  2️⃣  Backup existing config file$NC"
    Write-Host "$BLUE  3️⃣  Modify machine code config$NC"
    Write-Host "$BLUE  4️⃣  Display operation completion info$NC"
    Write-Host ""
    ...
    Write-Host "$BLUE  4️⃣  Restart Cursor to let it generate new config file$NC"
    Write-Host "$BLUE  5️⃣  Wait for config file generation to complete (max 45 seconds)$NC"
    Write-Host "$BLUE  6️⃣  Close Cursor process$NC"
    Write-Host "$BLUE  7️⃣  Modify newly generated machine code config file$NC"
...
if ($executeMode -eq "MODIFY_ONLY") {
    ...
    if (-not $envCheck.Success) {
        ...
        Write-Host "$BLUE  2️⃣  Manually launch Cursor once, then rerun the script$NC"

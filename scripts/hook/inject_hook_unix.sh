...
stop_cursor_process() {
    ...
    sleep 2
    log_info "Cursor process stopped"
}
...
main() {
    ...
    if check_already_injected "$main_js" && ! $FORCE; then
        log_warn "Hook already injected, no need to repeat"
        log_info "Use --force parameter to force re-injection"
        exit 0
    fi
